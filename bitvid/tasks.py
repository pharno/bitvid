...
def transcode(videotoken, height, codec):
    ...
    videoMedia = Media(videofile_original_location(video.token,video.originalmime))
    ...
    outpath = videofile_converted_location(video.token, height, codec_mime_mapping[codec])

    cmd = arguments.format(origpath=videoMedia.source_file,codec=codec,height=height,width=width).split(" ")
